...
this.updateTexture = function(e) {
    ...
    this.pass.material.defines.LUT_3D = "3D" === e.type;
    this.pass.material.defines.LUT_SIZE = e.size.toFixed(1);
    this.pass.material.defines.DOMAIN_MIN = `vec3(${e.domain[0].map(e=>e.toFixed(1)).join(",")})`;
    this.pass.material.defines.DOMAIN_MAX = `vec3(${e.domain[1].map(e=>e.toFixed(1)).join(",")})`;
...
this.unload = function(e) {
    this.lut && this.parentProject.assets.unload(this.lut);
    this.parentProject.assets.unload(this.vertShader);
    this.parentProject.assets.unload(this.fragShader);
};
this.update = function(e) {
    if(this.pass) {
        this.pass.enabled = 1 === this.properties.enabled.get(e) && null !== this.properties.lut.get()
    }
};
