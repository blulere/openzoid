...
function ffmpeg_run(opts)
{
  ...
  //don't exit after main finishes
  Module['noExitRuntime'] = true;

  Module['preRun'] = [ function() {

    //prevent default handler for stdin
    FS.init(function () { return null; }, null, null);

    //provide reference to heap
    if (Module['bufferRef'])
    {
      Module['bufferRef'](Module['buffer']);
    }

    if (Module['gotRemux'])
    {
      Module['gotRemux'](remux);
    }

    //create dev files
    /*
    var f0 = FS.makedev(64, 0);
    FS.registerDevice(f0,
    {
      read: Module['read0']
    });
    FS.mkdev("/in0", 0777, f0);

    var f1 = FS.makedev(65, 0);
    FS.registerDevice(f1,
    {
      read: Module['read1']
    });
    FS.mkdev("/in1", 0777, f1);

    var f2 = FS.makedev(66, 0);
    FS.registerDevice(f2,
    {
      read: Module['read2']
    });
    FS.mkdev("/in2", 0777, f2);

    */

    //create output directory
    FS.createFolder('/', 'output', true, true);

    if (Module['useDevFile'])
    {
      var f3 = FS.makedev(67, 0);
      FS.registerDevice(f3,
      {
        read: Module['read'],
        write: Module['write'],
        llseek: Module['seek']
      });
      FS.mkdev("/output/out", 0o0777, f3);
    }

    //add any symlinks to the input files
    // if (Module['links0'])
    // {
    //   for (var i = 0; i < Module['links0'].length; i++)
    //   {
    //     FS.symlink("/in0", Module['links0'][i]);
    //   }
    // }
    //
    // if (Module['links1'])
    // {
    //   for (var i = 0; i < Module['links1'].length; i++)
    //   {
    //     FS.symlink("/in1", Module['links1'][i]);
    //   }
    // }

    //add any files to the root directory
    if (Module['files'])
    {
      Module['files'].forEach(function(file)
      {
        FS.createDataFile('/', file.name, new Int8Array(file.data), true, true);
      });
    }
  } ];
  ...
  {
    if (Module['options'].decode)
    {
      //call asm decode function
      _decode();
    }
    else
    {
      var opts_ptr = _get_options();
      var opts = new Int32Array(Module['buffer'], opts_ptr, 13);

      opts[0] = Module['options'].haveVideo;
      opts[1] = Module['options'].haveAudio;
      opts[2] = Module['options'].frameWidth;
      opts[3] = Module['options'].frameHeight;
      opts[4] = Module['options'].frameRate;
      opts[5] = Module['options'].videoBitrate;
      opts[6] = Module['options'].audioBitrate;
      opts[7] = Module['options'].cpu_used;
      opts[8] = Module['options'].deadline;
      opts[9] = Module['options'].lag_in_frames;
      opts[10] = Module['options'].lossless;
      opts[11] = Module['options'].crf;
      opts[12] = Module['options'].video_codec;

      Module.stringToUTF8(Module['options'].filename, opts_ptr + 13*4, 32);

      //call asm encode function
      _encode();
    }
  };

  function remux(options)
  {
    FS.mkdir("/mux");
    FS.mkdir("/mux/in");
    FS.mount(WORKERFS, { blobs: [ options.file ] }, "/mux/in");

    var filename = "/mux/in/" + options.file.name;
    var name_ptr = allocate(intArrayFromString(filename), 'i8', ALLOC_NORMAL);

    var offset = _remux(name_ptr, options.startTime, options.duration, options.typeMask);
    ...
    var handle = FS.analyzePath("/mux");
    var buffers = getAllBuffers(handle);
    ...
    return { offset: offset, buffers: buffers };
  }

  function done()
  {
    //return with files from output directory (if any)
    var handle = FS.analyzePath('output');
    if (Module['return'])
      Module['return'](getAllBuffers(handle));
  }

  function get_audio_samples(data_ptr, num)
  {
    return Module['getAudioSamples'](data_ptr, num, _got_audio_samples);
  }

  function get_video_frame(data_ptr)
  {
    return Module['getVideoFrame'](data_ptr, _got_video_frame);
  }
...
function integrateWasmJS() {
    var wasmTextFile = "av.wast";
    var wasmBinaryFile = "av.wasm";
    var asmjsCodeFile = "av.temp.asm.js";
...
var ASM_CONSTS = [(function() {
    ...
    return get_video_frame($0)
}), (function($0, $1) {
    return get_audio_samples($0, $1)
})];
...
STATICTOP = STATIC_BASE + 1079872;
__ATINIT__.push({
    func: (function() {
        ___emscripten_environ_constructor()
    })
});
var STATIC_BUMP = 1079872;
...
Module["wasmTableSize"] = 2238;
Module["wasmMaxTableSize"] = 2238;
...
var _get_options = Module["_get_options"] = (function() {
    return Module["asm"]["_get_options"].apply(null, arguments)
});
var _got_audio_samples = Module["_got_audio_samples"] = (function() {
    return Module["asm"]["_got_audio_samples"].apply(null, arguments)
});
var _got_video_frame = Module["_got_video_frame"] = (function() {
    return Module["asm"]["_got_video_frame"].apply(null, arguments)
});
var _malloc = Module["_malloc"] = (function() {
    return Module["asm"]["_malloc"].apply(null, arguments)
});
var _remux = Module["_remux"] = (function() {
    return Module["asm"]["_remux"].apply(null, arguments)
});
